...
- **Docker base image now tracks Ubuntu 26.04 and FFmpeg 9.** `linuxserver/ffmpeg:latest` rebased from Ubuntu 24.04 / FFmpeg 8 to Resolute. Recordings, live transcode, concat, and `ffmpeg`/`ffprobe` in the container all use that FFmpeg 9 binary. Custom Stream Profile arguments that depended on FFmpeg 8-only options may need a review.
- **Comskip is now built from [erikkaashoek/Comskip](https://github.com/erikkaashoek/Comskip) `master` (0.83.001) instead of Ubuntu's 0.82.011 package.** Ubuntu 26.04 dropped the universe `comskip` package (it still depended on `libavcodec60`). The builder compiles current Comskip against distro libav (FFmpeg 8 shared libraries; linuxserver ships libav statically inside `ffmpeg` with no `.so` to link). Tagged V0.83 does not compile on gcc 15 or FFmpeg 8; switch to the next GitHub release tarball when they tag one that does. uWSGI now links `libpcre2` (`libpcre3` is gone on Resolute). The comskip binary is linked with DT_RPATH to distro libfontconfig so linuxserver's older `/usr/local/lib` does not win at runtime.
