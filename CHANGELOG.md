...
- **Base image build tracks Ubuntu 26.04 after `linuxserver/ffmpeg:latest` rebased to Resolute.** That release dropped `libpcre3` and the `comskip` package, so apt failed on amd64 and arm64. uWSGI now builds against `libpcre2`. Comskip is compiled in the builder from the latest GitHub release of [erikkaashoek/Comskip](https://github.com/erikkaashoek/Comskip) (the universe package is gone) and copied into the runtime image with `libargtable2-0`.
