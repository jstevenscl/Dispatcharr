...
- **VOD honors Redirect as the default stream profile.** When Redirect is the global default, VOD GET/HEAD (including XC) return HTTP 301 to the provider URL instead of proxying. Decided only on the first request (no `session_id`): an idle same-viewer/content fingerprint match adopts that proxy session instead; otherwise the client is handed off to a capacity-aware provider URL (no session mint, no slot hold, no upstream probe). Existing `session_id` URLs keep proxying. The Redirect check is Redis-cached; the idle-session scan runs only when Redirect is active. Stream settings note that Redirect as the default also redirects VOD.
