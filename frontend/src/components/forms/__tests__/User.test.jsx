...
vi.mock('@mantine/core', () => ({
  ...
  NumberInput: ({ label }) => <div><label>{label}</label><input /></div>,
  PasswordInput: ({ label, disabled }) => (
    <div><label>{label}</label><input type="password" disabled={disabled} /></div>
  ),
  Select: ({ label, disabled }) => (
    <div><label>{label}</label><select disabled={disabled} /></div>
  ),
  Stack: ({ children }) => <div>{children}</div>,
  Switch: ({ label }) => <div><label>{label}</label><input type="checkbox" /></div>,
  Tabs: ({ children }) => <div>{children}</div>,
  TabsList: ({ children }) => <div>{children}</div>,
  TabsPanel: ({ children, value }) => <div data-testid={`panel-${value}`}>{children}</div>,
  TabsTab: ({ children, value }) => <button data-testid={`tab-${value}`}>{children}</button>,
  TagsInput: ({ label }) => <div><label>{label}</label><input /></div>,
  Text: ({ children }) => <span>{children}</span>,
  TextInput: ({ label, value, disabled, rightSection }) => (
    <div>
      <label>{label}</label>
      <input data-testid={`textinput-${label}`} defaultValue={value} disabled={disabled} />
      {rightSection}
    </div>
  ),
  useMantineTheme: () => ({
...
describe('User', () => {
  ...
  describe('tabs', () => {
    ...
    it('hides Permissions tab for non-admin user', () => {
      ...
      render(
        <User isOpen={true} onClose={vi.fn()} user={makeRegularUser({ id: 99 })} />
      );
      expect(screen.queryByTestId('tab-permissions')).not.toBeInTheDocument();
    });
  });
  ...
  describe('admin-only fields', () => {
    it('shows Output Format Override for admin', () => {
      setupMocks({ authUser: makeAdminUser() });
      render(
        <User isOpen={true} onClose={vi.fn()} user={makeRegularUser({ id: 2 })} />
      );
    ...
    it('hides Output Format Override for non-admin', () => {
      setupMocks({ authUser: makeRegularUser() });
      render(
        <User isOpen={true} onClose={vi.fn()} user={makeRegularUser({ id: 5 })} />
      );
